...
struct OpArg
{
	...
	OpArg(u64 _offset, int _scale, X64Reg rmReg = RAX, X64Reg scaledReg = RAX)
	{
		operandReg = 0;
		scale = (char)_scale;
		offsetOrBaseReg = (char)rmReg;
		indexReg = (char)scaledReg;
	...
	void WriteSingleByteOp(XEmitter *emit, char op, X64Reg operandReg, int bits);
...
inline OpArg Imm8 (char imm)  {return OpArg(imm, SCALE_IMM8);}
...
private:
	...
	void WriteSimple1Byte(int bits, char byte, X64Reg reg);
	void WriteSimple2Byte(int bits, char byte1, char byte2, X64Reg reg);
	void WriteMulDivType(int bits, OpArg src, int ext);
	void WriteBitSearchType(int bits, X64Reg dest, OpArg src, char byte2);
	void WriteShift(int bits, OpArg dest, OpArg &shift, int ext);
	void WriteMXCSR(OpArg arg, int ext);
	void WriteSSEOp(int size, char sseOp, bool packed, X64Reg regOp, OpArg arg, int extrabytes = 0);
	void WriteNormalOp(XEmitter *emit, int bits, NormalOp op, const OpArg &a1, const OpArg &a2);

protected:
	inline void Write8(char value)   {*code++ = value;}
...
public:
	XEmitter() { code = NULL; }
	XEmitter(char *code_ptr) { code = code_ptr; }

	void WriteModRM(int mod, int rm, int reg);
	void WriteSIB(int scale, int index, int base);

	void SetCodePtr(char *ptr);
	void ReserveCodeSpace(int bytes);
	const char *AlignCode4();
	const char *AlignCode16();
	const char *AlignCodePage();
	const char *GetCodePtr() const;
	char *GetWritableCodePtr();
	...
	void JMP(const char * addr, bool force5Bytes = false);
	void JMP(OpArg arg);
...
#undef CALL
	...
	void J_CC(CCFlags conditionCode, const char * addr, bool force5Bytes = false);
	...
	void CMPSS(X64Reg regOp, OpArg arg, char compare);  
	void CMPSD(X64Reg regOp, OpArg arg, char compare);  
	...
	void CMPPS(X64Reg regOp, OpArg arg, char compare);  
	void CMPPD(X64Reg regOp, OpArg arg, char compare);
	...
	void SHUFPS(X64Reg regOp, OpArg arg, char shuffle);  
	void SHUFPD(X64Reg regOp, OpArg arg, char shuffle);  
	...
	void PEXTRW(X64Reg dest, OpArg arg, char subreg);
	void PINSRW(X64Reg dest, OpArg arg, char subreg);
	...
	void PSHUFLW(X64Reg dest, OpArg arg, char shuffle);
...
public:
	...
	{
		region_size = size;
		region = (char*)AllocateExecutableMemory(region_size);
		SetCodePtr(region);
	}
