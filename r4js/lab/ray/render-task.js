...
function normaliseVector(self) {
    ...
    var s1 = self[1];
    var s2 = self[2];
    var len = sqrt(s0 * s0 + s1 * s1 + s2 * s2);
    self[0] /= len;
    self[1] /= len;
    self[2] /= len;
    return self;
...
function Circle(pos, radius) {
    ...
    var f = function(ox, oy, oz, dx, dy, dz, near, far){
        ...
        var diry = oy - cy;
        var dirz = oz - cz;
        var B = dirx * dx + diry * dy + dirz * dz;
        var C = dirx * dirx + diry * diry + dirz * dirz;
        var t = B * B - C;
        if (t < near || t > far)
             return t;
        var dist = B - sqrt(D);
        f.normal = [(dist * dx + dirx) / radius,
                    (dist * dy + diry) / radius,
                    (dist * dz + dirz) / radius];
        return dist;
    }
...
onmessage = function(event) {
    try {
        ...
        if (message.type === "init") {
            ...
            processRenderCommand = message.processRenderCommand;
            postMessage("initDone");
        } else {
            result = processRenderCommand(message.command);
            postMessage("({jobid:" + message.jobid + ", result:" + result + "})");
        }
    } catch(e) {
        postMessage(e);
    }
